...
MAX_RECALL_ATTEMPTS = (
    3  # максимальное количество попыток перезвона при сброшенном вызове
)
...
# Call Content
CALL_CONTENT_APPLICANT_MALE_PROBABILITY = (
    50  # вероятность того что в блоке call content в поле male будет мужской пол
)
...
AVG_OPERATOR_WAIT_ANSWER_RECALL_TIME = (
    60  # секунды, время дозвона со стороны оператора при обратном вызове
)
...
AVG_CARD_CREATE_TIME = (
    75  # секунды, среднее время вызова/составления карточки/dtConnect - dtEndCall
)
...
AVG_EOS_CARD_CREATE_TIME = (
    300  # секунды, среднее время составления карточки со стороны службы/dtCreate
)
...
AVG_OPERATOR_RECALL_WAITING_TIME = (
    5  # секунды, среднее время задержки перед обратным вызовом/dtCall
)
...
AVG_DEPARTMENT_ANSWER_TIME = (
    15  # среднее время ответа службы(используется только в card04->dtConfirm)
)
DEPARTMENT_SCALE = 8
...
WORKING_CONDITIONS_CHARACTERS = [
    "нахождение объекта в огражденной",
    "охраняемой зоне",
    "наличие ворот",
    "шлагбаумов",
...
EVACUATIONS_POSSIBILITIES = [
    "наличие незадымленных лестничных клеток",
    "лестниц между балконами",
    "открытых галерей",
]
...
SUSPECT_MALE_GENDER_PROBABILITY = (
    50  # Вероятность того, что подозреваемый мужского пола
)
...
AMBULANCE_SHARE_MIN = 0
AMBULANCE_SHARE_MAX = 0
