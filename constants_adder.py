...
s = '''xml_count_per_file = 1  # Количество записей в одном файле
...
for el in s:
    if '_WORK_TYPE' in el:
        ss.append(el)
        new_var1 = f"{el.replace('WORK_TYPE', 'LOW').split('=')[0]}= 1"
        new_var2 = f"{el.replace('WORK_TYPE', 'HIGH').split('=')[0]}= 1"
        new_var3 = f"{el.replace('WORK_TYPE', 'LAMBDA').split('=')[0]}= 1"
        ss.append(new_var1)
        ss.append(new_var2)
        ss.append(new_var3)
        continue
