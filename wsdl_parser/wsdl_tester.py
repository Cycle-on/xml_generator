...
def get_types_from_wsdl(path: str, get_capital_fields: bool = False) -> dict[str, set] | list:
    cards = {}
    tree = ET.parse(path)
    root = tree.getroot()
...
def check_fields_by_file_path(file_path: str, wsdl_path: str) -> None:
    ...
    for ukio_fields in get_xml_tree_fields_by_path(file_path):
        ...
        for schema_name, schema_fields in ukio_fields.items():
            ...
            missed_fields[schema_name]['count'] = missed_fields[schema_name].get('count', 0) + 1
            ...
            for field in missed_card_fields:
                missed_fields[schema_name][field] = missed_fields[schema_name].get(field, 0) + 1
...
def main():
    check_fields_by_file_path('../files/file_2024-10-16_12-54-03/ukios_0.xml', 'wsdl_4_3.wsdl')
